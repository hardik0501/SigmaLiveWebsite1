...
export function LeaderContactModal({ leader, isOpen = true, onClose }: LeaderContactModalProps) {
  ...
  }, [leader, isOpen, onClose]);

  if (!leader || !isOpen) return null;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await submitLead({
        ...
        message: `[Recipient: Leader - ${leader.name} (${leader.designation})] ${formData.message}`,
      });
          ...
          {submitted ? (
            <div className="py-10 text-center space-y-4">
              ...
              <p className="text-sm text-sigma-stone-600 max-w-xs mx-auto">
                Thank you, <span className="font-semibold">{formData.name}</span>. Your message has been routed to{' '}
                <span className="font-semibold text-sigma-blue-700">{leader.name}</span>'s executive desk.
              </p>
            </div>
          ) : (
            <div>
              <span className="eyebrow text-sigma-blue-600">Executive Advisory</span>
              ...
              <div className="mt-4 p-3 bg-sigma-stone-100/80 border border-sigma-stone-200/80 rounded-2xl flex items-center gap-2 text-xs font-semibold text-sigma-graphite-900">
                <User className="h-4 w-4 text-sigma-blue-600 shrink-0" />
                <span>{leader.designation} — Sigma Group</span>
              </div>
              ...
              <form onSubmit={handleSubmit} className="mt-5 space-y-4">
                ...
                <div>
                  ...
                  <textarea
                    rows={3}
                    placeholder="e.g. Schedule a strategic partnership discussion or high-value portfolio consultation..."
