...
export function RequestPricePage() {
  ...
  useEffect(() => {
    document.title = 'Request Price & Cost Sheet | Sigma Homes India';
  }, []);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setLoading(false);
      navigate('/thank-you?type=price_request');
    } catch (err) {
      setLoading(false);
    }
  };
    ...
    <div className="min-h-screen bg-sigma-ivory-50 text-sigma-graphite-900 pt-32 pb-20 md:pt-40">
      <div className="container-content max-w-xl">
        <div className="text-center mb-8 space-y-2">
          <span className="eyebrow text-sigma-blue-600">Official Cost Sheet</span>
          ...
          <p className="text-xs md:text-sm text-sigma-stone-600">
            Request current unit pricing and cost sheets directly from Sigma Homes.
          </p>
        </div>
        ...
        <div className="bg-white rounded-3xl border border-sigma-stone-200/80 p-6 md:p-8 shadow-xl">
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            >
              {loading ? 'Requesting...' : 'Request Official Cost Sheet'}
            </button>
          </form>
        </div>
      </div>
